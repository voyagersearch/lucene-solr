  ...
  private static void addField(Document doc, SchemaField field, Object val, float boost, 
    ...
    for (IndexableField f : field.getType().createFields(field, val, boost)) {
      if (f != null) { // null fields are not added
        if (forInPlaceUpdate) {
          // createFields() generates more than one indexable fields. if this document is 
          // to be used for an in-place update, then include only the NumericDocValuesField
        ...
        for( Object v : field ) {
          ...
          if (sfield != null) {
            ...
            addField(out, sfield, v, applyBoost ? compoundBoost : 1f, 
                schema.getUniqueKeyField() != null && 
                name.equals(schema.getUniqueKeyField().getName())? false: forInPlaceUpdate);
          ...
          if (copyFields != null) {
            ...
            if (!(forInPlaceUpdate && name.equals(schema.getUniqueKeyField().getName()))) {
                ...
                final float destBoost = 
                    (destinationField.indexed() && !destinationField.omitNorms()) ?
                        ...
                        addField(out, destinationField, val, destBoost, 
                            destinationField.getName().equals(schema.getUniqueKeyField().getName())?
                                false: forInPlaceUpdate);
                        // record the field as having a value
